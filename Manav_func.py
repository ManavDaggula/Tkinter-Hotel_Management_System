...
def create_room_table(container,selected_option):
    ...
    lbl_header1 = tk.Label(container, text="Room No",bg="#0d3c59",fg="#ffffff")
    lbl_header1.grid(row=0,column=0)
    lbl_header2 = tk.Label(container, text="Availability",bg="#0d3c59",fg="#ffffff")
    lbl_header2.grid(row=0,column=1)
    lbl_header3 = tk.Label(container, text="Room Type",bg="#0d3c59",fg="#ffffff")
    lbl_header3.grid(row=0,column=2)
    lbl_header4 = tk.Label(container, text="Customer ID",bg="#0d3c59",fg="#ffffff")
    lbl_header4.grid(row=0,column=3)
    # ADDING THE ROWS
    row_count=1
    for row in result:
        # print(row)
        lbl1 = tk.Label(container,text=row[0],bg="#0d3c59",fg="#ffffff")
        lbl1.grid(row=row_count,column=0)
        lbl2 = tk.Label(container,text="Free",bg="#0d3c59",fg="#00ff00")
        if(row[2]!=None):
            lbl2['text']="Occupied"
            lbl2['fg']='#ffffff'
        lbl2.grid(row=row_count,column=1)
        lbl3 = tk.Label(container,text=row[1],bg="#0d3c59",fg="#ffffff")
        lbl3.grid(row=row_count,column=2)
        lbl4 = tk.Label(container, text=row[2],bg="#0d3c59",fg="#ffffff")
